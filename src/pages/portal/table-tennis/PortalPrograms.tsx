...
export default function PortalPrograms() {
  ...
  const activePrograms = userEnrollments.filter(p => p.programId.startsWith('tt-'));
  ...
  return (
    <div className="space-y-12 pb-12">
      <div className="card-premium border-red-tile bg-black/95">
        <h1 className="font-display text-3xl font-bold text-white tracking-tight">Table Tennis Programs</h1>
        <p className="text-sm text-white/70 font-bold mt-1">Manage your training sessions and track your match performance.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-12">
          ...
          <section className="space-y-6">
            <h2 className="text-xs font-black uppercase tracking-[0.2em] text-red-500/60 flex items-center gap-3">
              <span className="w-8 h-[1px] bg-red-500/20"></span> Active Enrollments
            </h2>
            ...
            <div className="grid gap-6">
              {activePrograms.length > 0 ? activePrograms.map((program, i) => (
                <motion.div 
                  ...
                  className="card-premium border-red-tile bg-black/95 flex flex-col group overflow-hidden relative"
                >
                  <div className="absolute top-0 right-0 w-32 h-32 bg-red-500/5 rounded-full -mr-16 -mt-16 blur-3xl group-hover:bg-red-500/10 transition-all duration-1000" />
                  
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-8">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-red-500/10 flex items-center justify-center shrink-0 group-hover:bg-red-500 transition-all shadow-lg shadow-black/20">
                          <Activity size={24} className="text-red-500 group-hover:text-white transition-all" />
                        </div>
                        <div>
                          <h3 className="font-display text-xl font-bold text-white group-hover:text-red-500 transition-colors">{program.title}</h3>
                          <p className="text-xs text-white/40 font-bold uppercase tracking-tight">{program.coach}</p>
                        </div>
                      </div>
                      <span className="text-[10px] font-black uppercase tracking-widest text-white bg-red-500 px-3 py-1.5 rounded-full shadow-lg shadow-red-500/20">
                        Active
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                      ...
                      <div className="p-5 rounded-2xl bg-white/3 border border-white/5 group-hover:border-white/10 transition-all">
                        ...
                        <p className="text-sm font-black text-red-500">{program.attendanceRate}%</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 mb-8">
                      <div className="flex justify-between text-[11px] font-black uppercase tracking-widest text-white/60">
                        ...
                        <span className="text-red-500">{program.progress}%</span>
                      </div>
                    ...
                    <div className="pt-6 border-t border-white/5 flex items-center justify-between">
                      ...
                      <button className="text-red-500 font-black text-xs uppercase tracking-widest hover:underline flex items-center gap-1.5 group/btn">
          ...
          {availablePrograms.length > 0 && (
            <section className="space-y-6">
              <h2 className="text-xs font-black uppercase tracking-[0.2em] text-red-500/60 flex items-center gap-3">
                <span className="w-8 h-[1px] bg-red-500/20"></span> Discover Other Programs
              </h2>
              ...
              <div className="grid gap-6">
                {availablePrograms.map((program, i) => (
                  <motion.div 
                    ...
                    viewport={{ once: true }}
                    transition={{ delay: i * 0.1 }}
                    className="card-premium border-white/5 bg-black/95 group hover:border-red-500/30 transition-all overflow-hidden relative"
                  >
                    <div className="flex flex-col md:flex-row gap-6">
                      ...
                      <div className="flex-1 flex flex-col">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <h3 className="text-lg font-bold text-white group-hover:text-red-500 transition-colors">{program.title}</h3>
                        ...
                        <div className="mt-auto flex items-center justify-between pt-4 border-t border-white/5">
                          ...
                          <button className="px-5 py-2 rounded-xl bg-red-500 text-white text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all shadow-glow shadow-red-500/20">
                            Enroll Now
                          </button>
                        </div>
                      </div>
        ...
        <div className="space-y-6">
          <div className="card-premium border-red-500/20 bg-red-500/5 shadow-2xl shadow-red-500/5">
            <h3 className="font-display text-lg font-bold text-white mb-6 flex items-center gap-2 uppercase tracking-tight">
              <TrendingUp size={20} className="text-red-500" /> Training Insights
            </h3>
            <div className="space-y-4">
              <div className="p-5 rounded-2xl bg-black/40 border border-white/5 group">
                <p className="text-xs font-black uppercase tracking-widest text-red-500 mb-2">Power Score</p>
                <p className="text-[11px] text-white/60 leading-relaxed font-medium">Your smash velocity has <span className="text-white">increased by 8%</span> this month.</p>
              </div>
              <div className="p-5 rounded-2xl bg-black/40 border border-white/5 group">
                <p className="text-xs font-black uppercase tracking-widest text-gold mb-2">Next Rank</p>
                <p className="text-[11px] text-white/60 leading-relaxed font-medium">Win <span className="text-white">2 more matches</span> to reach Silver League status.</p>
              </div>
            </div>
          </div>
          ...
          <div className="card-premium border-white/5 bg-black/80">
            <h3 className="font-display text-lg font-bold text-white mb-3 flex items-center gap-2 uppercase tracking-tight">
              <Star size={18} className="text-red-500" /> Ranking
            </h3>
            <p className="text-xs text-white/40 mb-6 font-bold leading-relaxed">Climb the global leaderboard and earn your pro-tier certifications.</p>
