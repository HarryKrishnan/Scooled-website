...
const pickleballPayments = [
  { id: "pp1", invoice: "INV-PB-001", date: "Mar 01, 2026", type: "Annual Membership", method: "UPI", amount: 15999, status: "Completed" },
  { id: "pp2", invoice: "INV-PB-002", date: "Feb 20, 2026", type: "Tournament Entry", method: "Credit Card", amount: 1500, status: "Completed" },
  { id: "pp3", invoice: "INV-PB-003", date: "Feb 15, 2026", type: "Paddle Rental", method: "UPI", amount: 500, status: "Completed" },
  { id: "pp4", invoice: "INV-PB-004", date: "Feb 10, 2026", type: "Social Play Fee", method: "Net Banking", amount: 800, status: "Completed" },
];
...
export default function PortalPayments() {
  ...
  const paymentMethods = [
    ...
    { id: "wallet", name: "Scooled Wallet", icon: Wallet, color: "text-emerald-500", bg: "bg-emerald-500/10" },
  ];
  ...
  const handleVerifyCoupon = () => {
    ...
    setTimeout(() => {
      if (couponCode.toUpperCase() === "PICKLE20") {
  ...
  const baseAmount = paymentType === "upgrade" && selectedPlan ? selectedPlan.price : 1500;
    ...
    <div className="space-y-6">
      <div className="card-premium border-green-tile bg-black/95 mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-sm text-white/70 font-bold mt-1">Manage your Pickleball billing history and secure payments.</p>
        </div>
        <button 
          onClick={() => setShowPaymentModal(true)}
          className="btn-primary flex items-center gap-2 px-8 py-2.5 shadow-lg shadow-emerald-500/20 shrink-0 bg-emerald-500 hover:bg-emerald-600"
        >
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card-premium border-green-tile bg-black/95 group">
          ...
          <div className="flex items-end justify-between">
            <p className="text-2xl font-bold text-white">₹22,299</p>
        ...
        <div className="card-premium border-gold-tile bg-black/95 group">
          ...
          <div className="flex items-end justify-between">
            <p className="text-2xl font-bold text-emerald-500">₹1,500</p>
                  ...
                  >
                    ...
                    <td className="py-5">
                      <span className="flex items-center gap-3 text-white/60 font-bold">
                        <div className="w-8 h-8 rounded-xl bg-white/5 flex items-center justify-center group-hover:bg-emerald-500/20 group-hover:text-emerald-500 transition-all">
                          <Icon size={14} />
                        </div>
                        {p.method}
                      </span>
                    ...
                    <td className="py-5 text-right">
                      <button className="p-2.5 rounded-xl hover:bg-emerald-500/10 hover:text-emerald-500 text-white/20 transition-all">
                        <Download size={18} />
                      </button>
                    </td>
                  </motion.tr>
            ...
            >
              <div className="p-8 pb-4 flex items-center justify-between border-b border-white/5">
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="font-display font-bold text-xl text-white">
                    {paymentStep === "type" ? "Pickleball Payment" : paymentStep === "upgrade" ? "Tier Upgrade" : "Complete Pay"}
                  </h3>
                </div>
              ...
              <div className="p-8 max-h-[70vh] overflow-y-auto">
                {paymentStep === "type" && (
                  <div className="space-y-4">
                    <button onClick={() => { setPaymentType("standard"); setPaymentStep("method"); }} className="card-premium border-white/5 bg-white/5 hover:border-emerald-500 transition-all group w-full text-left">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            <p className="font-bold text-lg text-white">Arena / Fee</p>
                            <p className="text-[10px] text-white/40 font-black uppercase tracking-widest mt-1">Pending dink fees</p>
                          </div>
                        </div>
                        <ChevronRight size={20} className="text-white/20 group-hover:text-emerald-500" />
                      </div>
                    </button>
                    <button onClick={() => { setPaymentType("upgrade"); setPaymentStep("upgrade"); }} className="card-premium border-gold-tile bg-emerald-500/5 hover:border-emerald-500 transition-all group w-full text-left">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-5">
                          <div className="w-14 h-14 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 group-hover:bg-emerald-500 group-hover:text-white transition-all">
                            <Zap size={28} />
                          ...
                          <div>
                            <p className="font-bold text-lg text-white">Upgrade Pickleball Plan</p>
                            <p className="text-[10px] text-emerald-500 font-black uppercase tracking-widest mt-1">Special gear access</p>
                          </div>
                        </div>
                        <ArrowUpRight size={20} className="text-emerald-500/20 group-hover:text-emerald-500" />
                      </div>
                    </button>
                  </div>
                )}
                ...
                {paymentStep === "upgrade" && (
                  <div className="space-y-4">
                    {memberships.filter(plan => plan.price > mockActive.price).map(plan => (
                      <button key={plan.id} onClick={() => { setSelectedPlan(plan); setPaymentStep("method"); }} className={`w-full p-6 text-left rounded-[2rem] border-2 transition-all group ${selectedPlan?.id === plan.id ? "border-emerald-500 bg-emerald-500/10" : "border-white/5 bg-white/5 hover:border-emerald-500/30"}`}>
                        <div className="flex justify-between items-center">
                          <div>
                            <p className={`font-bold ${selectedPlan?.id === plan.id ? "text-emerald-500" : "text-white"}`}>{plan.name}</p>
                ...
                {(paymentStep === "method" || paymentStep === "summary") && (
                  <div className="space-y-8">
                    ...
                    <div className="flex gap-2">
                       <input type="text" placeholder="PICKLE20" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} className="flex-1 px-5 py-4 rounded-2xl bg-black/40 border border-white/10 text-white font-bold text-sm focus:border-emerald-500 outline-none transition-all" />
                       <button onClick={handleVerifyCoupon} className="px-6 rounded-2xl bg-emerald-500 text-white text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 transition-all">Verify</button>
                    </div>
                    {couponStatus === "success" && <p className="text-[10px] font-black uppercase text-green-500 px-2 mt-2">Saved ₹{discount}!</p>}
                    <div className="space-y-4">
                      {paymentMethods.map(m => (
                        <button key={m.id} onClick={() => setSelectedMethod(m.id)} className={`w-full p-5 rounded-[2rem] border-2 flex items-center justify-between transition-all ${selectedMethod === m.id ? "border-emerald-500 bg-emerald-500/10" : "border-white/5 bg-white/5"}`}>
                          <div className="flex items-center gap-5">
                            <div className={`w-12 h-12 rounded-2xl ${m.bg} ${m.color} flex items-center justify-center`}><m.icon size={22} /></div>
                            <span className="font-bold text-white">{m.name}</span>
                          </div>
                          {selectedMethod === m.id && <Check size={18} className="text-emerald-500" />}
                        </button>
                      ))}
                    </div>
                  </div>
              ...
              <div className="p-8 pt-4 border-t border-white/5 bg-black/40">
                <button 
                  ...
                  onClick={() => resetModal()}
                  className="w-full py-5 rounded-2xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-[10px] font-black uppercase tracking-widest shadow-2xl shadow-emerald-500/20 hover:scale-[1.02] transition-all disabled:opacity-20"
                >
