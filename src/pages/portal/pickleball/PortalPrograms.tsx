...
export default function PortalPrograms() {
  ...
  const activePrograms = userEnrollments.filter(p => p.programId.startsWith('pb-'));
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-12">
          ...
          <section className="space-y-6">
            <h2 className="text-xs font-black uppercase tracking-[0.2em] text-emerald-500/60 flex items-center gap-3">
              <span className="w-8 h-[1px] bg-emerald-500/20"></span> Active Enrollments
            </h2>
            ...
            <div className="grid gap-6">
              {activePrograms.length > 0 ? activePrograms.map((program, i) => (
                <motion.div 
                  ...
                  className="card-premium border-green-tile bg-black/95 flex flex-col group overflow-hidden relative"
                >
                  <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/5 rounded-full -mr-16 -mt-16 blur-3xl group-hover:bg-emerald-500/10 transition-all duration-1000" />
          ...
          {availablePrograms.length > 0 && (
            <section className="space-y-6">
              <h2 className="text-xs font-black uppercase tracking-[0.2em] text-emerald-500/60 flex items-center gap-3">
                <span className="w-8 h-[1px] bg-emerald-500/20"></span> Discover Other Programs
              </h2>
              ...
              <div className="grid gap-6">
                {availablePrograms.map((program, i) => (
                  <motion.div 
                    ...
                    viewport={{ once: true }}
                    transition={{ delay: i * 0.1 }}
                    className="card-premium border-white/5 bg-black/95 group hover:border-emerald-500/30 transition-all overflow-hidden relative"
                  >
                    <div className="flex flex-col md:flex-row gap-6">
                      ...
                      <div className="flex-1 flex flex-col">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <h3 className="text-lg font-bold text-white group-hover:text-emerald-500 transition-colors">{program.title}</h3>
                        ...
                        <div className="mt-auto flex items-center justify-between pt-4 border-t border-white/5">
                          ...
                          <button className="px-5 py-2 rounded-xl bg-emerald-500 text-white text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all shadow-glow shadow-emerald-500/20">
                            Enroll Now
                          </button>
                        </div>
                      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="card-premium border-white/5 bg-black/80">
            <h3 className="font-display text-lg font-bold text-white mb-3 flex items-center gap-2 uppercase tracking-tight">
              <Star size={18} className="text-emerald-500" /> Badges
            </h3>
            <p className="text-xs text-white/40 mb-6 font-bold leading-relaxed">Unlock legendary pickleball badges by dominating on the court.</p>
